...
device = torch.device(
    'cuda' if torch.cuda.is_available() else 'cpu')  # 在模型定义的时候不使用这条语句，而是在训练循环中使用，这样可以确保模型和数据都被正确地移动到GPU上（如果可用）。在模型定义时，模型的参数默认是在CPU上创建的，如果直接将模型定义放在GPU上，可能会导致一些问题，例如在某些环境中可能无法正确识别GPU设备，或者在模型定义时就占用GPU资源，导致后续的训练过程出现问题。因此，建议在训练循环中使用这条语句来动态地检测和使用GPU设备，以确保模型和数据都能正确地利用GPU加速训练。
...
tokens = tokenizer(text, return_tensors='pt')['input_ids'][
    0]  # 使用分词器将文本转换为输入ID，并返回一个PyTorch张量，选择第一个元素（因为返回的是一个批次的输入ID），得到一个一维张量，表示文本的分词结果
...
class Pretraindataset(Dataset):
    ...
    def __getitem__(self,
                    idx):  # __getitem__方法根据给定的索引idx返回数据集中的一个样本，这里返回的是一个字典，包含输入ID和标签。输入ID是从块中获取的，而标签是输入ID的克隆，确保输入和标签是相同的。这种设计适用于语言模型的预训练任务，其中模型需要预测下一个标记，因此输入和标签是相同的。
...
for epoch in range(epochs):
    ...
    for step, batch in enumerate(trainloader):
        optimizer.zero_grad()
        input_ids = batch['input_ids'][:, :-1].to(
            device)  # 使用shifted input_ids作为模型的输入，shifted input_ids是将原始输入ID向右移动一个位置得到的，这样模型在训练过程中就可以学习预测下一个标记。具体来说，input_ids[:, :-1]表示去掉每个序列的最后一个标记，而labels=batch['labels'][:, 1:]表示去掉每个序列的第一个标记，这样输入和标签就对齐了，模型可以学习从输入预测标签。
        labels = batch['labels'][:, 1:].to(device)
        ...
        torch.nn.utils.clip_grad_norm_(model.parameters(),
                                       1.0)  # 梯度裁剪，防止梯度爆炸，clip_grad_norm_函数用于对模型参数的梯度进行裁剪，model.parameters()返回模型的所有参数，1.0是裁剪的最大范数值，这样可以确保在反向传播过程中，梯度的范数不会超过1.0，从而稳定训练过程。
        optimizer.step()
