...
def create_tokenizer_config(save_dir: str) -> None:  # 创建一个函数，接受一个字符串参数save_dir，表示保存配置文件的目录路径，函数没有返回值
    ...
    with open(os.path.join(save_dir, 'tokenizer_config.json'), 'w', encoding='utf-8') as f:
        json.dump(config, f, ensure_ascii=False,
                  indent=4)  # indent是用来设置JSON文件的缩进格式的参数，设置为4表示每个层级的内容都会缩进4个空格，这样可以使生成的JSON文件更易读和美观
...
def train_tokenizer(data_path: str, save_dir: str, vocab_size: int = 8192) -> None:
    ...
    tokenizer.pre_tokenizer = pre_tokenizers.ByteLevel(
        add_prefix_space=False)  # ByteLevel预分词器用于将输入文本分割成字节级别的单元，特别适用于处理BPE（Byte Pair Encoding）分词器。它能够正确处理文本中的特殊字符和空格，并且在分词过程中保持原始文本的结构和格式，add_prefix_space参数控制是否在输入文本前添加一个空格，这对于某些语言和分词策略可能是必要的，以确保正确的分词结果。
    ...
    trainer = trainers.BpeTrainer(
        ...
        min_frequency=2,
        # min_frequency参数指定了一个token在训练数据中至少出现的次数，只有当一个token在训练数据中出现的次数达到或超过这个值时，它才会被包含在最终的词汇表中。这个参数有助于过滤掉那些在训练数据中非常罕见的token，从而减少词汇表的大小，提高模型的效率和性能。
        show_progress=True,  # 在训练过程中显示进度条，帮助用户了解训练的进展情况，特别是在处理大型数据集时，这个功能可以提供有用的反馈，让用户知道训练还需要多长时间。
        initial_alphabet=pre_tokenizers.ByteLevel.alphabet()
        # ByteLevel预分词器的初始字母表，包含了所有可能出现在输入文本中的字节级别的字符，这些字符将被用作分词器训练的基础，以便在训练过程中能够正确地处理和分词输入文本中的各种字符。
    )

    # 训练tokenizer
    print(f'Training tokenizer with data from{data_path}')
    texts = read_texts_from_files(data_path)
    tokenizer.train_from_iterator(texts, trainer=trainer, length=os.path.getsize(
        data_path))  # length参数指定了训练数据的总大小（以字节为单位），这有助于训练器在处理大型数据集时更有效地管理内存和计算资源，同时也可以提供更准确的进度反馈，特别是在训练过程中显示进度条时，这个参数可以帮助用户了解训练的进展情况。
