...
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    return (x.unsqueeze(-2).expand(batch_size, seq_len, n_kv_heads, n_rep, head_dim).reshape(batch_size, seq_len,
                                                                                             n_kv_heads * n_rep,
                                                                                             head_dim))
...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    freqs = 1.0 / (theta ** (torch.arange(0, dim, 2)[:(
                dim // 2)].float() / dim))  # 对应的公式为：freqs = 1.0 / (theta ** (torch.arange(0, dim, 2)[:, (dim // 2)].float() / dim))，其中torch.arange(0, dim, 2)生成一个从0到dim-1的偶数序列，表示特征向量中实部的索引。[:, (dim // 2)]将这个序列调整为一个列向量，以便后续计算。然后，将这个列向量除以dim，并使用theta作为基数计算频率。
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    ...
    x.shape[1], x.shape[-1])  # 确保freqs_cis的形状与x的形状兼容，freqs_cis的形状应该是 (seq_len, dim)，其中seq_len是x的第二维长度，dim是x的最后一维长度

    # 将freqs_cis的形状调整为 (1, seq_len, 1, dim)，以便与输入张量x进行广播操作
    return freqs_cis.unsqueeze(0).unsqueeze(
        -2)  # 首先使用unsqueeze(0)在第0维添加一个新的维度，使得张量的形状变为 (1, seq_len, dim)。然后，使用unsqueeze(-2)在倒数第二维添加一个新的维度，使得张量的形状变为 (1, seq_len, 1, dim)，以适应输入张量x的形状，并允许在后续计算中进行广播操作。


# 实现旋转嵌入
def rotary_emb(xq: torch.Tensor, xk: torch.Tensor, freqs_cos: torch.Tensor, freqs_sin: torch.Tensor) -> tuple[
    torch.Tensor, torch.Tensor]:
    # 将旋转嵌入应用至q,k矩阵，使得在进行注意力计算的时候，模型能够利用位置信息来更好地理解序列中元素之间的相对位置关系，从而提高模型的性能。
    # 重塑q,k维度，分离实部和虚部
    xq_r, xq_i = xq.float().reshape(xq.shape[:-1] + (-1, 2)).unbind(
        -1)  # 这里reshape将输入张量xq的最后一个维度调整为2，表示实部和虚部。具体来说，xq.shape[:-1]表示输入张量xq的所有维度，除了最后一个维度。通过在最后一个维度上添加(-1, 2)，我们将最后一个维度调整为2，表示实部和虚部。然后，使用unbind(-1)将这个新的维度分离成两个独立的张量xq_r和xq_i，分别表示实部和虚部。
...
class Attention(nn.Module):
    def __init__(self, args: ModelConfig):
        ...
        self.wk = nn.Linear(args.dim, self.n_kv_heads * self.head_dim,
                            bias=False)  # wk和wv使用分组查询注意力机制，因此它们的输出维度是键值头的数量乘以每个头的维数，而不是总头数乘以每个头的维数。这是因为在分组查询注意力机制中，每个查询头对应多个键值头，所以键值头的数量通常小于总头数。最后通过广播机制共享键值头
        ...
        self.flash = hasattr(torch.nn.functional,
                             'scaled_dot_product_attention')  # 检查torch.nn.functional模块是否具有scaled_dot_product_attention函数，以确定是否可以使用flash attention。如果存在这个函数，说明当前的PyTorch版本支持flash attention，可以利用这个优化的注意力机制来提高计算效率和内存使用效率，特别是在处理长序列时。
        if not self.flash:
            print(
                '当前PyTorch版本不支持flash attention，将使用常规的注意力计算方法。请考虑升级到支持flash attention的版本以获得更好的性能。')
...
class MLP(nn.Module):
    def __init__(self, dim: int, hidden_dim: int, multiple_of: int, dropout: float):
        ...
        if hidden_dim is None:
            ...
            hidden_dim = multiple_of * ((
                                                    hidden_dim + multiple_of - 1) // multiple_of)  # 为了确保hidden_dim是multiple_of的倍数，使用了这个公式。首先，将hidden_dim加上multiple_of-1，以确保在除以multiple_of时能够正确地向上取整。然后，使用整数除法将这个值除以multiple_of，得到一个整数，表示hidden_dim需要增加多少个multiple_of才能成为一个倍数。最后，将这个整数乘以multiple_of，得到最终的hidden_dim值，这个值是原始hidden_dim的最小倍数。
    ...
    def forward(self, x):
        # 前向传播
        return self.w2(self.dropout(F.silu(self.w1(x)) * self.w3(
            x)))  # 在前向传播中，首先通过线性变换w1将输入张量x映射到隐藏层维度，然后应用SILU激活函数（也称为Swish激活函数）来引入非线性。接下来，通过线性变换w3将输入张量x映射到隐藏层维度，并与之前的激活结果进行逐元素乘法操作，以实现门控机制。最后，通过线性变换w2将结果映射回输入维度，并应用dropout以防止过拟合并提高模型的泛化能力。
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, freqs_cos: torch.Tensor, freqs_sin: torch.Tensor):
        # 进行前向传播
        h = x + self.attention(self.attention_norm(x), freqs_cos,
                               freqs_sin)  # 这里使用了pre-norm结构，即在进行注意力计算之前先对输入张量x进行归一化处理。通过将输入张量x传递给attention_norm层进行归一化，可以帮助模型更好地稳定训练过程，并提高模型的性能。
...
class Transformer(PreTrainedModel):
    ...
    def __init__(self, args: ModelConfig = None):
        ...
        self.register_buffer('freqs_cos', freqs_cos,
                             persistent=False)  # 注册为缓冲区，表示这些张量不需要被优化器更新，但它们是模型的一部分，并且在保存和加载模型时会被包含在内。persistent=False表示这些缓冲区不会被保存到模型的状态字典中
        self.register_buffer('freqs_sin', freqs_sin, persistent=False)

        self.apply(
            self._init_weights)  # 参数的初始化，使用apply方法将_init_weights函数应用于模型的所有子模块，以确保所有的权重和偏置都被正确地初始化。这是一个常见的做法，可以帮助模型更快地收敛并提高性能。
        ...
        for pn, p in self.named_parameters():  # 遍历模型的所有参数，pn是参数的名称，p是参数的张量。通过named_parameters()方法，可以获取模型中每个参数的名称和对应的张量，以便进行特定的初始化操作。
            if pn.endswith('w3.weight') or pn.endswith(
                    'wo.weight'):  # endwith方法检查参数名称是否以指定的字符串结尾，这里检查是否以'w3.weight'或'wo.weight'结尾，以确定是否需要对这些特定的权重参数进行特殊的初始化处理。这里对这两个投影至输入维度的线性层进行特殊的初始化的原因是，这些层在模型中起着关键的作用，直接影响模型的输出和性能。通过对这些层进行特殊的初始化，可以帮助模型更快地收敛并提高性能，特别是在训练初期阶段。
        ...
        self._no_split_modules = [name for name, _ in
                                  self.named_modules()]  # _no_split_modules是一个列表，包含模型中所有模块的名称。通过named_modules()方法，可以获取模型中每个模块的名称和对应的模块对象。这个列表可以用于指定在模型并行处理或分布式训练过程中不需要进行切分的模块，以确保这些模块在不同设备之间保持完整。
    ...
    def forward(self, tokens: torch.Tensor, targets: Optional[torch.Tensor] = None, **kwargs) -> torch.Tensor:
        ...
        h = self.norm(
            h)  # 在所有的解码器层处理完输入张量h之后，使用LayerNorm进行归一化处理，以帮助模型更好地稳定训练过程，并提高模型的性能。通过对输出张量h进行归一化，可以确保每个位置的特征向量具有相似的尺度，从而有助于模型更有效地学习输入数据中的模式和关系。

        if targets is not None:
            logits = self.output(h)
            self.last_loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)), targets.reshape(-1),
    ...
    def generate(self, idx, stop_id=None, max_new_tokens=256, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            idx_cond = idx if idx.size(1) <= self.args.max_seq_len else idx[:,
                                                                        -self.args.max_seq_len:]  # 进行上下文截断，当序列过长的时候，选择保留最近的max_seq_len个token

            logits = self(
                idx_cond).logits  # self(idx_cond)用法是调用模型的前向传播函数，传入当前的输入序列idx_cond，并获取输出对象中的logits属性。这个logits属性包含了模型在当前输入序列下对下一个 token 的预测分数，这些分数可以用于后续的采样过程来生成新的 token。
            logits = logits[:, -1,
                     :]  # 只取最后一个位置的logits，logits维度变为 (batch_size, vocab_size)，表示模型对下一个 token 的预测分数。通过选择最后一个位置的logits，我们可以专注于当前时间步的输出，从而提高生成效率和准确性，因为在生成任务中，我们通常只关心当前时间步的输出，而不需要计算整个序列的输出。

            if temperature == 0.0:
                _, idx_next = torch.topk(logits, k=1,
                                         dim=-1)  # top_k函数有两个参数：k和dim。k参数指定了要返回的最大值的数量，这里设置为1，表示只返回最大的一个值。返回值是一个元组，包含了最大值和对应的索引。通过使用_来忽略最大值，只保留索引idx_next，这个索引表示了在logits张量中具有最高分数的token的位置。这个位置对应于模型预测的下一个 token 的索引，可以用于生成新的 token。
            else:
                ...
                if top_k is not None:
                    v, _ = torch.topk(logits, k=min(top_k, logits.size(
                        -1)))  # 取出logits中概率最高的top_k个值，v是这些值的张量，_是对应的索引。通过使用min(top_k, logits.size(-1))，确保在top_k大于词汇表大小时不会出现错误，因为logits.size(-1)表示词汇表的大小。
                    logits[logits < v[:, [-1]]] = -float(
                        'inf')  # 将logits中不在top_k范围内的值设置为负无穷，这样在后续的softmax计算中，这些值的概率将接近于零，从而限制了生成文本的选择范围，提高生成文本的质量和相关性。
                probs = F.softmax(logits, dim=-1)
                idx_next = torch.multinomial(probs,
                                             num_samples=1)  # 多项式采样，probs是一个形状为 (batch_size, vocab_size) 的张量，表示每个 token 的概率分布。通过使用torch.multinomial函数，可以根据这个概率分布随机采样一个 token 的索引，num_samples=1表示每个样本只采样一个 token。配合温度控制和top_k过滤，这个采样过程可以生成多样化且相关性较高的文本输出。

            if idx_next == stop_id:
                break

            idx = torch.cat((idx, idx_next),
                            dim=1)  # 将新生成的 token 的索引 idx_next 连接到当前的输入序列 idx 上，形成一个新的输入序列，以便在下一次迭代中继续生成下一个 token。通过使用 torch.cat 函数，可以沿着指定的维度（这里是 dim=1，即序列长度维度）将 idx 和 idx_next 连接起来，从而更新输入序列以包含新生成的 token。
...
def eval_tokenizer(tokenizer_path: str) -> None:
    ...
    try:
        tokenizer = AutoTokenizer.from_pretrained(
            tokenizer_path)  # AutoTokenizer是transformers库中的一个类，用于自动加载预训练的tokenizer。通过调用from_pretrained方法，并传入tokenizer_path参数，可以从指定的路径加载预训练的tokenizer对象。这个对象包含了用于文本处理和编码的各种功能，如分词、编码、解码等，可以用于后续的文本处理和模型输入准备。
    ...
    print(
        f"Special token IDs: {tokenizer.all_special_ids}")  # all_special_ids是一个列表，包含了tokenizer中所有特殊token的ID。这些ID对应于特殊token在词汇表中的位置，可以用于在编码和解码过程中正确处理这些特殊token。例如，在语言模型中，特殊token可能包括开始标记（<s>）、结束标记（</s>）、填充标记（<pad>）等，这些标记在生成文本时具有特定的功能和意义。通过查看all_special_ids，可以了解这些特殊token在词汇表中的位置，从而更好地理解和使用tokenizer的功能。
    ...
    print("Decoded text matches original:",
          decoded == prompt)  # 通过比较解码后的文本（decoded）与原始提示文本（prompt），可以验证编码和解码过程是否正确地保留了文本的内容和结构。如果两者完全匹配，说明tokenizer在编码和解码过程中没有丢失信息或引入错误，从而确保了文本处理的准确性和一致性。
    ...
    decoded = tokenizer.decode(
        encoded)  # 在这里，我们将测试文本test_text编码成input_ids，然后再解码回文本。通过比较解码后的文本与原始测试文本，可以验证tokenizer是否正确处理了特殊token（如<|im_start|>和<|im_end|>），并确保这些特殊token在编码和解码过程中得到了正确的保留和处理。这对于模型在处理包含特殊token的输入时能够正确理解和生成文本非常重要。
    print(f"Original: {test_text}")
    print(f"Decoded:  {decoded}")
    print("Special tokens preserved:",
          decoded == test_text)  # 通过比较解码后的文本与原始测试文本，可以验证tokenizer是否正确处理了特殊token（如<|im_start|>和<|im_end|>），并确保这些特殊token在编码和解码过程中得到了正确的保留和处理。这对于模型在处理包含特殊token的输入时能够正确理解和生成文本非常重要。
